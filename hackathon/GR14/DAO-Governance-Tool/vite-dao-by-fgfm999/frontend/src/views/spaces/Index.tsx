...
const SpaceIndex = ({vcInstance, callContract, setState, viteApi, networkType}: Props) => {
  const connectedAccount = useMemo(() => vcInstance?.accounts[0], [vcInstance]);

  const contractQuery = useMemo(() => {
    if (viteApi) {
      console.log('viteApi', viteApi)
      return new SpaceFactoryQuery(viteApi, SpaceFactoryContract.address[networkType], SpaceFactoryContract.abi)
    }
  }, [viteApi])
  ...
  useEffect(() => {
    if (contractQuery) {
      contractQuery.getSpacesPaging([0, 100]).then(async (res: any) => {
        ...
        spacesSet(spaceResult.filter((item: any) => item !== undefined) as Array<SpaceType>)
      })
    }
  }, [contractQuery])
