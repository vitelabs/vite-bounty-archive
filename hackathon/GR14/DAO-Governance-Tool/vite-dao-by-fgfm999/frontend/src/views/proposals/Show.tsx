...
import {State, Proposal as ProposalType, Space as SpaceType} from 'utils/types';
...
const ProposalShow = ({vcInstance, viteApi, callContract, setState}: Props) => {
  ...
  const [voted, votedSet] = useState<{ forVotes?: number, againstVotes?: number }>({})
  const [amount, amountSet] = useState<string>('')
  const [accountBalance, accountBalanceSet] = useState<{ balance: string, unreceived: string } | null>()
  ...
  const formatDate = useCallback((timestamp?: string): string => {
    return timestamp ? moment(parseInt(timestamp) * 1000).format('YYYY/MM/DD') : ''
  }, [])

  const proposalState = proposal?.executed === '1' ? 'Exceuted' : proposal?.canceled === '1' ? 'Canceled' : 'Active'
        ...
        <Grid container spacing={3}>
          <Grid item xs={9}>
            <Paper className="proposal-content">
              ...
              <div className='meta'>
                ...
                <span>by <em>{shortenAddress(proposal.proposer)}</em></span>
              </div>
            ...
            <Paper className="vote">
              ...
              <div className='vote-content'>
                <FormControl fullWidth>
                  ...
                  <TextField id='amount' value={amount} onChange={amountChange} label="Amount" variant="outlined"
                             fullWidth/>
                </FormControl>

                <div className='action-wrapper'>
                  <Button className='btn-vote' onClick={voteProposal} variant='contained'>Vote</Button>
                </div>
              </div>
            </Paper>
          </Grid>
          ...
          <Grid item xs={3}>
            <Paper className='proposal-card'>
              ...
              <List>
                <ListItem className='proposal-card-item' alignItems="flex-start">
