...
const Router = ({setState, vcInstance, networkType, toast}: Props) => {
  ...
  useEffect(() => {
    if (vcInstance) {
      ...
      subscribe('newLogs', vcInstance.accounts[0]).then((event: any) => {
  ...
  const callContract = useCallback(
    (
      ...
      const methodAbi = contract.abi.find((x: any) => x.name === methodName && x.type === 'function');
    ...
    <BrowserRouter>
      <PageContainer>
        <Routes>
          ...
          <Route path="/spaces/:address/proposals/" element={<ProposalIndex/>}/>
          <Route path="/spaces/:address/proposals/new" element={<ProposalNew/>}/>
          <Route path="/spaces/:address/proposals/:id" element={<ProposalShow/>}/>
        </Routes>
      </PageContainer>
