...
describe('test englishAuction', function () {
  before(async function () {
    ...
    // compile
    console.log("prepare english auction contract");
    const compiledContracts = await vite.compile('englishAuction.solpp');
    expect(compiledContracts).to.have.property('EnglishAuction');
    contract = compiledContracts.EnglishAuction;
    // deploy
  ...
  describe('bid a price for a nft', function (){
    ...
    it('bid a price lower than reverse price', async function (){
      try{
        ...
        expect(await contract.query('getAucPriceInfoById', ['1'])).to.be.deep.equal(['1000', '0', '1500', '100', '0']);
      }
    });

    it('bid a price large than reverse price', async function (){
      try{
        await contract.call('bidForNft', ['1', '3000'], {caller:alice});
      }finally{
        expect(await contract.query('getAucPriceInfoById', ['1'])).to.be.deep.equal(['1000', '0', '3000', '100', '0']);
      }      
    });
    ...
    it('bid a price large than reverse price', async function (){
      try{
        ...
        expect(await contract.query('getAucPriceInfoById', ['2'])).to.be.deep.equal(['1000', '0', '3000', '100', '0']);
      }      
    });    
    ...
    it('bid a price for an erc20 payment', async function (){
      try{
        ...
        expect(await contract.query('getAucPriceInfoById', ['5'])).to.be.deep.equal(['1000', '0', '3000', '100', '0']);
      }        
    });
  });
  ...
  describe('cancel a auction', function (){
    it('cancel auction 1', async function (){
      try{
        await contract.call('cancelAuction', ['1'], {caller:deployer});
      }finally{
        expect(await contract.query('getAucPriceInfoById', ['1'])).to.be.deep.equal(['1000', '2000', '3000', '100', '2']);
      }   
    });
    ...
    it('cancel auction 5', async function (){
      try{
        await contract.call('cancelAuction', ['5'], {caller:deployer});
      }finally{
        expect(await contract.query('getAucPriceInfoById', ['5'])).to.be.deep.equal(['1000', '2000', '3000', '100', '2']);
      }         
    });        
  });

  describe('claim the nft after the auction success', function () {
    it('claim failed for not success', async function () {
      try{
        await expect(contract.call('claimAuction', ['1'], {caller:alice})).to.eventually.be.rejectedWith("revert");
      }finally{
        expect(await contract.query('getAucPriceInfoById', ['1'])).to.be.deep.equal(['1000', '2000', '3000', '100', '2']);
      }
    });
  ...
  describe('get auction paginations indexs by page size and page count', function () {
    it('get auction paginations indexs by size 10, page 0, type 0, owner false', async function () {
      expect(await contract.query('getAuctionPaginations', ['10', '0', '0', false])).to.be.deep.equal([['2','4'],'2']);
    });
    ...
    it('get auction paginations indexs by size 10, page 0, type 2, owner false', async function () {
      expect(await contract.query('getAuctionPaginations', ['10', '0', '2', false])).to.be.deep.equal([['1','3','5'],'3']);
    });
    ...
    it('get auction paginations indexs by size 10, page 0, type 5, owner false', async function () {
      expect(await contract.query('getAuctionPaginations', ['10', '0', '5', false])).to.be.deep.equal([[],'0']);
    });  

    it('get auction paginations indexs by size 10, page 0, type 6, owner false', async function () {
      expect(await contract.query('getAuctionPaginations', ['10', '0', '6', false])).to.be.deep.equal([[],'0']);
    });        

    it('get auction paginations indexs by size 10, page 0, type 6, owner false', async function () {
      expect(await contract.query('getAuctionPaginations', ['10', '0', '7', false])).to.be.deep.equal([['1','3','5'],'3']);
    });                        
  });  
