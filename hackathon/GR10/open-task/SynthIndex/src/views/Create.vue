<template>
  <Page>
    ...
    <div class="d-flex flex-items-center px-4 px-md-0 mb-3">
      <h4 v-text="$t('Fee')" class="flex-auto" />
    </div>
    <div class="mb-4">
      <input
        class="input pool-input text-right"
...
export default {
  ...
  created() {
    ...
    const sETH = getTokenBySymbol('sETH').address;
    const sBTC = getTokenBySymbol('sBTC').address;
    this.tokens = [sETH, sBTC];
    // weights contain percentage values - denorms are calculated later
    Vue.set(this.weights, sETH, '60');
    Vue.set(this.weights, sBTC, '40');
    this.loading = false;
  },
  computed: {
    pool() {
